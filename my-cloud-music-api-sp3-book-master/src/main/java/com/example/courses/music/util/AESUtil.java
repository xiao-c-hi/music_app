...
public class AESUtil {
    ...
    private static String aes(String data, int mode) {
        try {
            ...
            //random.setSeed(AES128_KEY.getBytes(Constant.CHARSET));
            //generator.init(128, random);
            //SecretKey secretKey = generator.generateKey();
            //byte[] keyBytes = secretKey.getEncoded();

            //创建key
            SecretKeySpec key = new SecretKeySpec(AES128_KEY.getBytes(Constant.CHARSET), "AES");
            ...
            //需要一个向量iv
            //可增加加密算法的强度
            IvParameterSpec iv = new IvParameterSpec(AES128_IV.getBytes(Constant.CHARSET));
            ...
            if (mode == Cipher.ENCRYPT_MODE) {
                //加密
                byte[] result = cipher.doFinal(data.getBytes(Constant.CHARSET));
                ...
                //转为字符串
                return new String(result,Constant.CHARSET);
            }
        } catch (Exception e) {
