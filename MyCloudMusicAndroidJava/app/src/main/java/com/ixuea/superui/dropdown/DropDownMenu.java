package com.ixuea.superui.dropdown;
...
public class DropDownMenu extends ConstraintLayout {
    ...
    private void setUpPopupMenuViews(List<View> popupViews) {
        ...
        if (menuHeightPercent != 0f)
            popupMenuHeight = (int) (ScreenUtil.getScreenHeight(getContext()) * menuHeightPercent);
     ...
     * 获取指定位置tab
     */
    public void setTabText(String title, int index) {
        ((TextView) tabMenuView.getChildAt(index)).setText(title);
    }

    /**
     * 获取指定位置tab
     */
    public void setTabText(int title, int index) {
        ((TextView) tabMenuView.getChildAt(index)).setText(title);
    }
